...
CHANGELOG=$(git log --no-merges --pretty=%s "$PREVIOUS_TAG".."$CURRENT_TAG" --invert-grep --grep="Bomb" --grep "Retirement notice" --grep "Deprecation notice" --grep "Reduce Sentry usage and auth0 refresh token scheduler" --grep "skip ci" --grep "skip changelog" --grep "SKIP CI" --grep "SKIP CHANGELOG" | sed -e 's/^/- /' | sed -E 's/\(#[0-9]+\)//')
